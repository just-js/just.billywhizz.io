  ...
  tokenize (buf) {
    ...
    for (const b of u8) {
      ...
      if (inDirective) {
        ...
        if (c === '{' && last === '{') {
          if (end - start > 2) {
            this.tokens.push({ type: 'string', value: this.decoder.decode(u8.subarray(start, end - 1)) })
          }
          inDirective = true
          directive = {}
          inName = true
...
function compile (template, root = '', opts = {}) {
  const { plugins = {}, rawStrings = false } = opts
  const tokenizer = new Tokenizer()
  tokenizer.tokenize(encoder.encode(template))
  const parser = new Parser(root, rawStrings)
  parser.plugins = plugins
  parser.all(tokenizer.tokens)
  const fn = new Function(parser.args, parser.source.join('\n'))
  const call = (...args) => fn.call(args[0], [...args.slice(1)])
