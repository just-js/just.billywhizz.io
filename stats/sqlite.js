...
function createEnvironment (dbName, memory, heap, malloc, options = {}) {
  ...
  const environment = {
    ...
    wasm_wal_handler: (i0, i1, i2, pages) => {
      ...
      console.log(`wal_handler ${name} : ${pages}`)
      return constants.SQLITE_OK
    },
...
*/
    ...
    wasm_http_get_bytes: (i0, i1, start, end) => {
      ...
      if (env.openFiles[path].wal) {
        page = (start / (4096n + BigInt(walFrameHeaderSize)))
//        console.log(`wasm_http_get_bytes wal.page ${page}: ${start}, ${end - start}`)
        heap.set(env.wal.bytes.subarray(Number(start), Number(end)), i1)
        all_pages.set(page, env.wal.bytes.slice(Number(start), Number(end)))
        return constants.SQLITE_OK
      } else {
//        console.log(`wasm_http_get_bytes db.page ${page}: ${start}, ${end - start}`)
      }
      if (env.openFiles[path].journal) {
        return constants.SQLITE_OK
      }
//      console.log('wasm_http_get_bytes')
      const { bitmap } = env
      let origEnd = end
      if (end === 100n) end = 4096n
      const range = `bytes=${start}-${end - 1n}`
      if (env.cache.has(`${name}.${range}`)) {
        all_pages.set(page, env.cache.get(`${name}.${range}`))
      ...
      if (env.inMemory) {
        ...
        all_pages.set(page, heap.slice(i1, i1 + 4095))
        } else {
      ...
      if (end === 4096n && cache) {
        ...
        all_pages.set(page, env.cache.get(`${name}.bytes=0-4095`))
        return constants.SQLITE_OK
      }
      if (size === 4096) {
...
//        console.log(`http.get ${range}`)
    ...
    wasm_http_set_bytes: (i0, i1, amount, offset) => {
      ...
      const name = getName(path)
      const env = createDatabase(name)
      // todo: first 32 bytes of wal is the header
      // todo: we should be able to calculate exactly where we are in a particular page with offset and amount
      if (env.openFiles[path].wal) {
//        console.log(`wal.set ${offset} ${bytes.length}`)
//      console.log(`wasm_http_set_bytes.wal ${offset}, ${amount}`)
        if (amount === 24) {
          const bytes = heap.subarray(i1, i1 + amount)
          const page = bytes[3] + (bytes[2] << 8)
//          console.log(page)
//          console.log(all_pages.get(page))
//          console.log(heap.subarray(i1, i1 + amount))
          // this is the page number that is being updated
          // we can compare this page to the original (if we have it)
  //        console.log(bytes)
        }
...
//    const len = (str.length << 2) + 1
    ...
    const target = new Uint8Array(memory.buffer, ret, len)
...
from 
  ...
  join ${name}.pragma_threads()
    ...
    walInfo: (name = 'main') => {
      ...
      const consolidated = database.wal.file.consolidate(database.wal.file.readFrames(mxFrame))
      const delta = database.wal.file.serialize(consolidated)
      return {
        pageSize, checkpointSequence,
        counter, mxFrame: consolidated.length, nPage, backfill, backfillAttempt,
        stats, delta
      }
    },
    page: (name = 'main', page_num = 0) => {
      const database = db.databases[name]
      const start = page_num * 4096
      const end = start + 4096 - 1
      const cache_key = `${name}.bytes=${start}-${end}`
      if (database.cache.has(cache_key)) {
        return { page_num, bytes: database.cache.get(cache_key) }
      }
      return { page_num, bytes: null }
    },
    checkpoint: (name = 'main') => {
      const esp = stack.save()
      const plogSize = new Pointer(memory, stack.alloc(4))
      const pframesCheckpointed = new Pointer(memory, stack.alloc(4))
      const rc = ex.sqlite3_wal_checkpoint_v2(handle, CString(name), checkpoint.SQLITE_CHECKPOINT_FULL, plogSize.p, pframesCheckpointed.p)
      const database = db.databases[name]
      const { checkpointSequence } = database.wal.file
      const logSize = plogSize.get()
      const framesCheckpointed = pframesCheckpointed.get()
      stack.restore(esp)
      return { rc, logSize, framesCheckpointed, checkpointSequence }
    },
    status: () => {
      const results = []
...
//  const memory = new WebAssembly.Memory({ initial: 4096, maximum: 4096, shared: false })  
  const memory = new WebAssembly.Memory({ initial: 1024, maximum: 1024, shared: false })  
  ...
  if (wasmPath.constructor.name === 'String') {
    ...
    const mod = new WebAssembly.Module(wasmPath)
...
onmessage = async e => {
  ...
  if (e.data.page) {
    ...
    const page_num = e.data.page_num || 0
    postMessage(db.page(name, page_num))
    return
  }
